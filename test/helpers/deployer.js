...
class Deployer {
  ...
  async freshDeploy(owner) {
    ...
    this.stakingNFT = await contractFactories.StakingNFT.deploy('TOK Validator', 'MV')
  ...
  async deployTOKWeth() {
    const TOKWeth = await contractFactories.TOKWETH.deploy()
    await Promise.all([
      this.mapToken(TOKWeth.address, TOKWeth.address, false /* isERC721 */),
      this.updateContractMap(ethUtils.keccak256('wethToken'), TOKWeth.address)
    ])
    return TOKWeth
  }
  ...
  async deployTOKToken() {
    if (!this.globalTOK) throw Error('global TOK token is not initialized')
    ...
    await this.globalTOK.childToken.deposit(childToken.address, web3.utils.toHex(value))
  ...
  async initializeChildChain(options = { updateRegistry: true }) {
    ...
    let childTOKTokenAddress = utils.ChildTOKTokenAddress
    if (!this.globalTOK) {
      // MRC20 comes as a genesis-contract at utils.ChildTOKTokenAddress
      if (hre.__SOLIDITY_COVERAGE_RUNNING) {
        childTOKTokenAddress = (await contractFactories.MRC20.deploy()).address
      }

      this.globalTOK = { childToken: await contractFactories.MRC20.attach(childTOKTokenAddress) }
      const TOKOwner = await this.globalTOK.childToken.owner()
      if (TOKOwner === '0x0000000000000000000000000000000000000000') {
        // TOK contract at 0x1010 can only be initialized once, after the bor image starts to run
        await this.globalTOK.childToken.initialize(childSigner0, utils.ZeroAddress)
      }
    }
    if (this.registry) {
      ...
      this.globalTOK.rootERC20 = await this.deployTestErc20({
        mapToken: true,
        childTokenAdress: childTOKTokenAddress
      })
    }
