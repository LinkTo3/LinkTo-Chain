...
contract DepositManager is DepositManagerStorage, IDepositManager, ERC721Holder {
    ...
    // new: governance function to migrate TOK to POL
    function migrateTOK(uint256 _amount) external onlyGovernance {
        _migrateTOK(_amount);
    }

    function _migrateTOK(uint256 _amount) private {
        IERC20 TOK = IERC20(registry.contractMap(keccak256("TOK")));
        address polygonMigration = registry.contractMap(keccak256("polygonMigration"));

        // check that _amount is not too high
        require(TOK.balanceOf(address(this)) >= _amount, "amount exceeds this contract's TOK balance");

        // approve
        TOK.approve(polygonMigration, _amount);

        // call migrate function
        ILinkToMigration(polygonMigration).migrate(_amount);
    }
    ...
    function transferAssets(address _token, address _user, uint256 _amountOrNFTId) external isPredicateAuthorized {
        ...
        if (registry.isERC721(_token)) {
            ...
            // new: pay out POL when TOK is withdrawn
            if (_token == registry.contractMap(keccak256("TOK"))) {
                require(
    ...
    function _createDepositBlock(address _user, address _token, uint256 _amountOrToken, uint256 _depositId) internal {
        address TOK = registry.contractMap(keccak256("TOK"));

        // new: auto-migrate TOK to POL
        if (_token == TOK) {
            _migrateTOK(_amountOrToken);
        }
        // new: bridge POL as TOK, child chain behaviour does not change
